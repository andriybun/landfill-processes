...
addpath('../../Common');
addpath(genpath('../../Solute Transport/Solute Transport 1D'));
...
diffCoeff = 1e-4; % [1e-3, 5e-3];
exchCoeff = 1e-3; % [1e-1, 5e-1];
...
kSat = BoundaryPar.kSurf;             % m^3 water per m^2 soil per day
...
figure(3);
plotyy(tRange, qOut, tRange, mOut);

figure(4);
plot(tRange, mOut ./ qOut);
%% TODO: save qIn, cIni, cOut, qOut, mOut to file